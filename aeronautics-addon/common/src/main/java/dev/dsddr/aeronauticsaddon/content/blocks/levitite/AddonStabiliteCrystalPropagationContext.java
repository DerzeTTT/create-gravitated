...
public class AddonStabiliteCrystalPropagationContext implements CrystalPropagationContext {
    @Override
    ...
    public void onCrystallize(final Level level, final BlockPos pos) {
        ...
        if (!level.isClientSide) {
            AeroSoundEvents.LEVITITE_BLEND_CRYSTALLIZE.play(level, null, pos, 1.0f, 1.05f);
    ...
    public BlockState getCrystalBlockState(final Level level, final BlockPos pos) {
        return AddonBlocks.STABILITE.getDefaultState();
    }
    ...
    public boolean matchesCatalystBlock(final Level level, final BlockPos fluidPos, final BlockPos catalystPos, final BlockState catalystState) {
        return catalystState.is(Blocks.FIRE)
                && AddonCrystalCatalystHelper.hasCardinalCatalysts(level, fluidPos, AddonTags.BlockTags.STABILITE_CATALYZER);
    }
